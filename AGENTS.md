...
Run `composer install` once to pull dependencies. Use `make test` for the default PHPUnit run, or `vendor/bin/phpunit` if you need custom flags. `make tests` executes the matrix against PHP 8.2–8.4 via Docker Compose. Static and correctness tooling include `make csdiff` (style check), `make csfix` (auto-fix), `make stan` (PHPStan analysis), `make rector` (automated refactors), and `make infection` (mutation testing, requires Xdebug). Generate local coverage with `make coverage` and view the HTML artefact in `build/coverage/index.html`. For profiling SQL-heavy changes, `make bench` renders aggregate benchmarks to `build/bench.html`.
